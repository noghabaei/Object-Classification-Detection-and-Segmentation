...
def log(logger, img, label, pred, global_step):
    ...
    ax.add_artist(plt.Circle(WH2*(label[0].cpu().detach().numpy()+1), 20, ec='g', fill=False, lw=5))
    ax.add_artist(plt.Circle(WH2*(pred[0].cpu().detach().numpy()+1), 20, ec='r', fill=False, lw=5))
