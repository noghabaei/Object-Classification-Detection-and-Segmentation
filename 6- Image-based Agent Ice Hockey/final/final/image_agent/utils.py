    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None):
        ...
        for t in range(max_frames):
            ...
            if verbose:
                ...
                ax.add_artist(plt.Circle(WH2*(1+self._to_image(kart.location, proj, view)), 20, ec='b', fill=False, lw=1.5))
                ax.add_artist(plt.Circle(WH2*(1+self._to_image(aim_point_world, proj, view)), 20, ec='r', fill=False, lw=1.5))
                if planner:
                    ap = self._point_on_track(kart.distance_down_track + TRACK_OFFSET, track)
                    ax.add_artist(plt.Circle(WH2*(1+aim_point_image), 20, ec='g', fill=False, lw=1.5))
                plt.pause(1e-3)
