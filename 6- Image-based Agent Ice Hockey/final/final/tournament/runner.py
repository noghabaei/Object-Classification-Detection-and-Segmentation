...
class Match:
    ...
    def run(self, team1, team2, num_player=1, max_frames=MAX_FRAMES, max_score=3, record_fn=None, timeout=1e10,
        ...
        for it in range(max_frames):
            ...
            # n_images = 10000
            # #len(args.track)
            # images_per_track = n_images // 1
            #
            # data_callback = collect
            ...
            #
            # TRACK_OFFSET = 15
            # aim_point_world = _point_on_track(kart.distance_down_track + TRACK_OFFSET, TRACK_NAME)
            ...
            #     data_callback(it, np.array(race.render_data[0].image), aim_point_image)
            #
            # ##########################################
