...
def train(args):
    ...
    size_loss = torch.nn.MSELoss()
    #size_loss2 = torch.nn.MSELoss()
    global_step = 0
    for epoch in range(args.num_epoch):
        model.train()

        for img, gt_size in train_data:
            img, gt_size = img.to(device), gt_size.to(device)

            #size_w, _ = gt_det.max(dim=1, keepdim=True)

            size = model(img)
            loss_val = size_loss(size,gt_size)
            #loss_val2 = size_loss2(size, gt_size)
            ...
            if train_logger is not None and global_step % 100 == 0:
                log(train_logger, img, gt_size, size, global_step)
        ...
        if valid_logger is None or train_logger is None:
            print('epoch %-3d loss %f' % (epoch, loss_val))
    save_model(model)
